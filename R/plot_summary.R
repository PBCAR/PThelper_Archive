...
#' of either the overall sample or by a grouping variable.
#'
 ...
 ##### --- GROUP SUMMARY STATISTICS
    ...
    pt_plot <- ggplot2::ggplot(pt_summ, ggplot2::aes(x = factor(price), y = mean, group = group, colour = group)) +
      ...
      ggplot2::geom_segment(pt_max_summ[(pt_max_summ$measure=="Mean \u00B1 Std Error of Consumption"),],
                            mapping = ggplot2::aes(xend = factor(price), y = -Inf, yend = mean), colour = "#999999", linewidth = 0.5, linetype = "dashed") +
      ggplot2::geom_segment(pt_max_summ[(pt_max_summ$measure=="Mean \u00B1 Std Error of Consumption"),],
                            mapping = ggplot2::aes(x = -Inf, xend = factor(price), yend = mean), colour = "#999999", linewidth = 0.5, linetype = "dashed") +
      ...
      ggplot2::geom_point(pt_max_summ, mapping = ggplot2::aes(x = factor(price), y = mean),
                          size = 3.5, show.legend = F, colour = "#000000") + ggplot2::coord_cartesian(clip = "off")
